import { drawChar, drawText } from './font'
...
const MS_BETWEEN_CHARS = 50
const NUMBER_OF_LINES = 2
const CHARS_BY_LINE = 24
const LINESGAP = 10
const PADDING_Y = 10
const PADDING_X = 10
const FONTWIDTH = 8
...
export class Dialog {
	...
	constructor(params: DialogParams) {
		...
		this.#boxHeight =
			NUMBER_OF_LINES * 8 + PADDING_Y * 2 + LINESGAP * (NUMBER_OF_LINES - 1)
		this.#boxY = this.#canvas.height - this.#boxHeight - 8
		this.#boxWidth = CHARS_BY_LINE * 8 + PADDING_X * 2
	...
	next() {
		...
		if (this.#currentChunk?.length === 0 || this.#currentChunk === undefined) {
			...
			this.#lineCursor++
			if (this.#lineCursor >= NUMBER_OF_LINES) {
				this.#lineCursor = 0
	...
	#update = (now: number) => {
		this.#requestAnimationFrameId = requestAnimationFrame(this.#update)
		if (now - this.#lastTime < MS_BETWEEN_CHARS) return
		this.#lastTime = now
		if (
			this.#currentChunk?.length === 0 &&
			this.#lineCursor < NUMBER_OF_LINES - 1
		) {
			this.#lineCursor++
			this.#currentChunk = this.#text?.shift()
		}
	...
	#render = () => {
		...
		this.#lines.forEach((line, i) => {
			...
			const lineY = this.#boxY + PADDING_Y + 8 * i + i * LINESGAP
	...
	*#textSlicer(text: string) {
		...
		while (words.length > 0) {
			...
			if (slice.join(' ').length >= CHARS_BY_LINE)
				words.unshift(slice.pop() ?? '')
			let result = slice.join(' ')
			yield result
			lineIndex = (lineIndex + 1) % NUMBER_OF_LINES
			slice = []
		}
	}
